...
export const useWelcomeStore = defineStore('welcome', {
  state: () => ({
    data: [
      { title: 'Documentation', text: 'Vue’s <a href="https://vuejs.org/" target="_blank" rel="noopener">official documentation</a> provides you with all information you need to get started.' },
      {
        title: 'Tooling',
  ...
  actions: {
    async save({ key = 'welcome', value }) {
      await localSave.set('welcomeData', key, value)
    },
    async load() {
      let local = await localSave.get('welcomeData', 'welcome')
      if (!local || !local.data) {
        await this.save({ value: 'This is an about page' })
        local = await localSave.get('welcomeData', 'welcome')
      }
